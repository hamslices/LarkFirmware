...
{
    if (HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_1) != HAL_OK) { return HAL_ERROR; }
    if (HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_2) != HAL_OK) { return HAL_ERROR; }
    if (HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_3) != HAL_OK) { return HAL_ERROR; }
...
static void set_pwm_gamma_corrected(uint16_t r, uint16_t g, uint16_t b)
{
  __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_3, gamma_lut_12bit[r]);
  __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_2, gamma_lut_12bit[g]);
  __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, gamma_lut_12bit[b]);
}
